...
def fetch_imgs_path(path, class_encoding, equal_ratio_to_healthy, shuffle):
    ...
    if class_encoding == 'binary':
        temp_dict = {'unused':[], 'used':[]}
        if equal_ratio_to_healthy == True:
            ratio = int(len(imgs_path_dict['used']) / (len(class_folders) - 1))
            for folder, img_files  in imgs_path_dict.items():
                if folder == 'used':
                    temp_dict['used'] = random.sample(imgs_path_dict[folder], len(imgs_path_dict['used']))
                else:
                    temp_dict['unused'] = temp_dict['unused'] + random.sample(imgs_path_dict[folder], ratio)
            ...
            for folder, img_files  in imgs_path_dict.items():
                if folder == 'used':
                    temp_dict['used'] = imgs_path_dict[folder]
                else:
                    temp_dict['unused'] = temp_dict['unused'] + imgs_path_dict[folder]
